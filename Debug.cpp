...
void DebugInfo::update(int fps, double frame_time, double angle, double distance, sf::Vector2i mouse_pos, sf::Vector2f circle_pos, int number_of_bullets,
	int number_of_enemies, double sin, double cos, int sizeof_bullet, int sizeof_enemy, bool& pause)
{
	if (!pause)
	{
		DebugInfo::debug_str = "";
		DebugInfo::debug_str.append("FPS: " + std::to_string(fps) + ", " + std::to_string(frame_time) + " ms")
			.append("\nangle(rad): " + std::to_string(angle))
		...
		if (sin < 0) DebugInfo::debug_str.append("\nsin: " + std::to_string(sin));
		else DebugInfo::debug_str.append("\nsin:  " + std::to_string(sin));
		if (cos < 0) DebugInfo::debug_str.append("\ncos: " + std::to_string(cos));
		else DebugInfo::debug_str.append("\ncos:  " + std::to_string(cos));

		DebugInfo::debug_str.append("\nnumber of:")
